...
interface IAavePool { function flashLoanSimple(address receiverAddress, address asset, uint256 amount, bytes calldata params, uint16 referralCode) external; }
...
contract FlashArbExecutor {
    ...
    constructor(address _pool, address _routerA, address _routerB) {
        ...
        owner = msg.sender; pool = _pool; allowedRouter[_routerA] = true; allowedRouter[_routerB] = true;
        emit RouterPermissionChanged(_routerA, true); emit RouterPermissionChanged(_routerB, true);
    }
    function setRouter(address router, bool allowed) external onlyOwner { if (router == address(0)) revert InvalidPath(); allowedRouter[router] = allowed; emit RouterPermissionChanged(router, allowed); }
    ...
    function startArbitrage(address asset, uint256 amount, address routerA, address[] calldata pathA, uint256 minOutA, address routerB, address[] calldata pathB, uint256 minOutB, uint256 minProfit, uint256 deadline) external onlyOwner {
        if (asset == address(0) || amount == 0) revert InvalidAmount(); if (minProfit == type(uint256).max) revert InvalidProfitFloor(); if (deadline < block.timestamp) revert DeadlineExpired(); if (tradeInProgress) revert TradeInProgress();
    ...
    function executeOperation(address asset, uint256 amount, uint256 premium, address initiator, bytes calldata params) external returns (bool) {
        ...
        uint256 required = preLoanBalance + amount + premium; uint256 finalAmount = IERC20(asset).balanceOf(address(this));
        if (finalAmount < required) revert NotProfitable(finalAmount, required);
        uint256 surplus = finalAmount - required; if (surplus < minProfit) revert NotProfitable(finalAmount, required + minProfit);
        _forceApprove(asset, pool, amount + premium); emit ArbitrageExecuted(asset, amount, premium, surplus); return true;
    }

    function withdraw(address token, uint256 amount, address recipient) external onlyOwner { if (recipient == address(0)) revert InvalidPath(); if (!IERC20(token).transfer(recipient, amount)) revert TokenTransferFailed(); emit ProfitWithdrawn(token, amount, recipient); }
    function withdrawETH(address payable recipient) external onlyOwner { if (recipient == address(0)) revert InvalidPath(); (bool ok,) = recipient.call{value: address(this).balance}(""); if (!ok) revert TokenTransferFailed(); }
    function _validateTrade(address asset, address routerA, address[] memory pathA, address routerB, address[] memory pathB) internal view {
        if (!allowedRouter[routerA]) revert RouterNotAllowed(routerA); if (!allowedRouter[routerB]) revert RouterNotAllowed(routerB); if (pathA.length < 2 || pathB.length < 2) revert InvalidPath(); if (pathA[0] != asset || pathB[pathB.length - 1] != asset || pathA[pathA.length - 1] != pathB[0]) revert InvalidPath();
        for (uint256 i; i < pathA.length; ++i) if (pathA[i] == address(0)) revert InvalidPath(); for (uint256 i; i < pathB.length; ++i) if (pathB[i] == address(0)) revert InvalidPath();
    }
    function _forceApprove(address token, address spender, uint256 amount) internal { if (!IERC20(token).approve(spender, 0)) revert TokenApprovalFailed(); if (!IERC20(token).approve(spender, amount)) revert TokenApprovalFailed(); }
