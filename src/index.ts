...
const amounts = (process.env.CANDIDATE_AMOUNTS_WETH || '0.05,0.1,0.25,0.5,1,2,5').split(',').map((v) => parseEther(v.trim())).filter((v) => v > 0n);
...
async function quote(router: Address, amountIn: bigint, path: Address[]): Promise<bigint> {
  const result = await publicClient.readContract({ address: router, abi: V2_ROUTER_ABI, functionName: 'getAmountsOut', args: [amountIn, path] });
...
async function estimateGasCost(account: Address, opportunity: NonNullable<Awaited<ReturnType<typeof findOpportunity>>>, minProfit: bigint, deadline: bigint) {
  ...
  return { gas, gasCost: (gas * gasPrice * gasSafetyBps) / 10_000n };
}
async function findOpportunity() {
  const premiumBps = await flashPremiumBps();
  const poolLiquidity = await publicClient.readContract({ address: ADDRESSES.weth, abi: ERC20_ABI, functionName: 'balanceOf', args: [ADDRESSES.aavePool] });
  const paths = [
    { routerA: ADDRESSES.camelotV2, routerB: ADDRESSES.uniswapV2 },
    { routerA: ADDRESSES.uniswapV2, routerB: ADDRESSES.camelotV2 },
  ] as const;
  let best: { amount: bigint; first: bigint; final: bigint; expectedProfit: bigint; estimatedPremium: bigint; routerA: Address; routerB: Address; pathA: Address[]; pathB: Address[] } | null = null;
  for (const amount of amounts) {
    ...
    for (const route of paths) {
      try {
        ...
        const expectedProfit = final > amount + estimatedPremium ? final - amount - estimatedPremium : 0n;
        if (expectedProfit >= targetProfit && (!best || expectedProfit > best.expectedProfit)) best = { amount, first, final, expectedProfit, estimatedPremium, routerA: route.routerA, routerB: route.routerB, pathA: [ADDRESSES.weth, ADDRESSES.usdc], pathB: [ADDRESSES.usdc, ADDRESSES.weth] };
      } catch (error) { console.warn('quote failed', error instanceof Error ? error.message : error); }
    }
  }
  return best;
}
async function executeOpportunity(opportunity: NonNullable<Awaited<ReturnType<typeof findOpportunity>>>) {
  ...
  const simulation = await publicClient.simulateContract({ account, address: executor, abi: EXECUTOR_ABI, functionName: 'startArbitrage', args: [ADDRESSES.weth, opportunity.amount, opportunity.routerA, opportunity.pathA, minOut(opportunity.first), opportunity.routerB, opportunity.pathB, minOut(opportunity.final), requiredProfitFloor, deadline] });
  if (dryRun) { console.log(`DRY RUN: simulation passed | gas=${gas} reserve=${formatEther(gasCost)} WETH`); return; }
...
async function main() {
  ...
  while (true) {
    try { const opportunity = await findOpportunity(); if (opportunity) { console.log({ amount: formatEther(opportunity.amount), expectedFinal: formatEther(opportunity.final), expectedProfitAfterPremium: formatEther(opportunity.expectedProfit), premium: formatEther(opportunity.estimatedPremium), route: `${opportunity.routerA} -> ${opportunity.routerB}` }); if (executor) await executeOpportunity(opportunity); } }
